...
**Q: Can I use `require` or `import` in the code block?**

A: No. The code block is executed in a sandboxed environment, and does not have access to the `require` or `import` functions. If you need to `require` or `import`, [use an External Call Block](./external-call.mdx) instead, or [create a Lawme plugin](../user-guide/plugins/creating-plugins.md).
